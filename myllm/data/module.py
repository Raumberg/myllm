...
class DataModule:
    ...
    def setup(self) -> None:
        ...
        # ------------------------------------------------------------------
        # Completion-only (v2, smarter)
        # ------------------------------------------------------------------
        elif collator_type in {"completion_only_v2", "completion_v2", "trl"}:
            from myllm.data.collators import DataCollatorForCompletionOnlyLMV2

            self.collator = DataCollatorForCompletionOnlyLMV2(
                tokenizer=self.tokenizer,
        ...
        # ------------------------------------------------------------------
        # TRL completion-only collator
        # ------------------------------------------------------------------
        # elif collator_type in {"trl_completion", "trl_completion_only", "trl"}:
        #     # ------------------------------------------------------------------
        #     # HuggingFace TRL collator (official implementation)
        #     # ------------------------------------------------------------------
        #     try:
        #         from trl import DataCollatorForCompletionOnlyLM as TrlCollator  # type: ignore
        #     except ImportError as e:  # pragma: no cover
        #         raise RuntimeError(
        #             "trl is not installed – run `pip install trl` or choose a different collator_type"
        #         ) from e

        #     self.collator = TrlCollator(
        #         tokenizer=self.tokenizer,
        #         # TRL expects `response_template` kwarg; falls back to default when None
        #         response_template=self.data_cfg.collator.template,
        #         mlm=False,
        #         ignore_index=self.data_cfg.collator.ignore_index,
        #     )
    ...
    def train_dataloader(self, shuffle: bool = False, num_workers: int = 2) -> DataLoader:  # noqa: D401
        return DataLoader(
            self.train_ds,
    ...
    def eval_dataloader(self, shuffle: bool = False, num_workers: int = 2) -> Optional[DataLoader]:  # noqa: D401
    ...
    def test_dataloader(self, shuffle: bool = False, num_workers: int = 2) -> Optional[DataLoader]:  # noqa: D401
